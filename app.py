...
class SimulationEngine:
    ...
    def swap_leaders(self, platoon_id_a, platoon_id_b):
        ...
        with self._lock:
            ...
            try:
                ...
                # Simpan posisi/kecepatan sebelum swap (Fase 3: Eksekusi Atomik Â§6.4.3)
                ...
                def make_leader(vehicle, platoon_id, position, velocity):
                    ...
                    vehicle.transfer_cooldown = 2.0   # Fase 5: Stabilisasi 2s (Â§6.4.5)
                    vehicle.fsm.reset()

                # Fase 3: Eksekusi atomik (window < 100ms â€” Eq. 24)
                make_leader(leader_a, platoon_id_b, pos_b, vel_b)
                make_leader(leader_b, platoon_id_a, pos_a, vel_a)

                # Fase 4: Pembaruan referensi member (Â§6.4.4)
    ...
    def step(self):
        ...
        for pid_inj, pvs_inj in platoons.items():
            pvs_inj_sorted = sorted(pvs_inj, key=lambda v: -v.position)
    ...
    def _check_collisions(self):
        ...
        for i in range(len(self.vehicles)-1):
            l, f = self.vehicles[i], self.vehicles[i+1]
            if l.platoon_id != f.platoon_id or l.collided or f.collided: continue
            gap = l.position - f.position - l.length
            rv  = f.velocity - l.velocity   # positif = mendekat
            cause = None
            if gap <= 0:
                cause = f"Physical overlap (gap={gap:.2f}m)"
            elif gap < self.config.get('min_safe_spacing', 1.0) and rv > 1.0:
                # Gap < 1m DAN masih mendekati = tabrakan tak terhindarkan
